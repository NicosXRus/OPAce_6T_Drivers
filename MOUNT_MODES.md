...
| **Использование `loop`-устройств** | ❌ Не используются | 🛠️ 1 виртуальный диск в `/mnt/ace6tdv` |
...
* **Принцип работы:** Упаковывает файлы модуля в сжатый образ EROFS (`odm.img`), монтирует его в `/mnt/ace6tdv/odm` и накладывает слои OverlayFS поверх системного раздела `/odm`. Для изолируемых каталогов использует пустышку-заглушку `empty`.
...
| **`loop` devices usage** | ❌ Not used | 🛠️ 1 virtual disk in `/mnt/ace6tdv` |
...
#### 2. ⚡ OverlayNative Mode
* **How it works:** Packs module files into a compressed EROFS image (`odm.img`), mounts it to `/mnt/ace6tdv/odm`, and applies OverlayFS layers over `/odm`. Uses a dynamic `empty` directory to isolate target folders.
...
| **Uso de `loop`** | ❌ No se utilizan | 🛠️ 1 disco virtual en `/mnt/ace6tdv` |
...
#### 2. ⚡ OverlayNative Mode
* **Funcionamiento:** Empaqueta los archivos en una imagen comprimida EROFS (`odm.img`), la monta en `/mnt/ace6tdv/odm` y aplica capas OverlayFS sobre `/odm`.
...
| **`loop` 设备使用** | ❌ 不使用 | 🛠️ `/mnt/ace6tdv` 中的 1 个虚拟盘 |
...
#### 2. ⚡ OverlayNative 模式
* **工作原理：** 将模块文件打包为 LZ4HC 压缩的 EROFS 镜像 (`odm.img`)，挂载至 `/mnt/ace6tdv/odm`，并通过 OverlayFS 覆盖于 `/odm` 分区之上。
...
| **`loop` デバイス** | ❌ 未使用 | 🛠️ `/mnt/ace6tdv` 内の1つの仮想ディスク |
...
#### 2. ⚡ OverlayNative モード
* **仕組み:** ファイルを圧縮 EROFS イメージ (`odm.img`) にパックし、`/mnt/ace6tdv/odm` にマウントして OverlayFS レイヤーを `/odm` に適用します。
...
| **Використання `loop`** | ❌ Не використовуються | 🛠️ 1 віртуальний диск у `/mnt/ace6tdv` |
...
#### 2. ⚡ OverlayNative Mode
* **Принцип роботи:** Упаковує файли у стиснений образ EROFS (`odm.img`), монтує його в `/mnt/ace6tdv/odm` та накладає шари OverlayFS поверх системного розділу `/odm`.
...
| **`loop` սարքեր** | ❌ Չեն օգտագործվում | 🛠️ 1 վիրտուալ սկավառակ `/mnt/ace6tdv`-ում |
...
* **Աշխատանքի սկզբունքը:** Ֆայլերը սեղմում է EROFS պատկերի մեջ (`odm.img`), տեղադրում `/mnt/ace6tdv/odm`-ում և OverlayFS շերտերը տարածում `/odm` բաժնի վրա:
